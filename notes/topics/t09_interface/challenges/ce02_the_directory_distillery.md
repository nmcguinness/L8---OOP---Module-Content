...
Over the years, different departments—**HR**, **IT**, **Sales**, and **Management**—have maintained their own contact spreadsheets, each with slightly different formatting styles, typos, spacing, missing fields, and even conflicting versions of the same person’s details.
...
# Appendix B — Utility: Map Department → Avatar Label/Emoji
...
> ⚠️ These are simplified and are **not** production-grade validators.  
...
- `^\+?` → optional leading `+`  
- `[0-9]` → require at least one digit after that  
- `[0-9 \-]{5,}` → then at least 5 more characters that are digits, spaces, or dashes  
